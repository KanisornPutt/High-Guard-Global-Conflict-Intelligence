...
news_table = dynamodb.Table(
    os.environ.get("NEWS_TABLE_NAME", "newsSummary")
)  # Source — articles from Summary 01
country_table = dynamodb.Table(
    os.environ.get("COUNTRY_TABLE_NAME", "countrySummary")
)  # Target — country briefings
...
def get_existing_country_summary(country: str) -> dict:
    ...
    if items:
        print(
            "Found existing summary for "
            + country
            + " — lastUpdated: "
            + items[0].get("lastUpdated", "unknown")
        )
        return items[0]
...
def update_last_checked(country: str, last_updated: str) -> None:
    ...
    country_table.update_item(
        Key={"country": country, "lastUpdated": last_updated},
...
def save_country_summary(
    country: str, summary: dict, priority_tier: str = "unknown", tier_count: int = 0
) -> None:
...
def check_new_news_after(country: str, after_timestamp: str) -> bool:
    ...
    response = news_table.scan(
        FilterExpression=Attr("country").eq(country)
        & Attr("fetchedAt").gt(after_timestamp),
        Limit=1,
    )
    has_new = len(response.get("Items", [])) > 0
    print(
        "New news fetched after "
        + after_timestamp
        + " for "
        + country
        + ": "
...
def call_bedrock(country: str, events: list, priority_tier: str = "unknown") -> dict:
    ...
    for attempt in range(3):
        try:
            ...
            print(
                "Raw response: "
                + str(response_body if "response_body" in locals() else "N/A")
            )
            raise
    ...
    if not records and ("requestContext" in event or "httpMethod" in event):
        ...
        if not country:
            return {
                ...
                "body": json.dumps(
                    {"message": "Please provide 'country' in the request body"}
                ),
            }
